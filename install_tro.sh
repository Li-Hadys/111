...
Port88=`netstat -tlpn | awk -F '[: ]+' '$1=="tcp"{print $5}' | grep -w 88`
Port4433=`netstat -tlpn | awk -F '[: ]+' '$1=="tcp"{print $5}' | grep -w 4433`
if [ -n "$Port88" ]; then
    process88=`netstat -tlpn | awk -F '[: ]+' '$5=="88"{print $9}'`
    red "==========================================================="
    red "检测到88端口被占用，占用进程为：${process88}，本次安装结束"
    red "==========================================================="
    exit 1
fi
if [ -n "$Port4433" ]; then
    process4433=`netstat -tlpn | awk -F '[: ]+' '$5=="4433"{print $9}'`
    red "============================================================="
    red "检测到4433端口被占用，占用进程为：${process4433}，本次安装结束"
    red "============================================================="
    exit 1
fi
...
start_menu(){
    ...
    green " 此脚本为 hady 的，集成BBRPLUS加速及MAC客户端 "
    green "                 "
    green " ===================================="
    blue " 声明："
    red " *请不要在任何生产环境使用此脚本"
    red " *请不要有其他程序占用88和4433端口"
